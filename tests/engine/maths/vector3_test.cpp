...
namespace test
{
    void vector3Test()
    {
        ...
        LOG_CHANNEL( "TEST", "<<Scaled test>>" );
        {
            vec3f v3 = v1;
            v3 *= -0.2f;
            LOG_CHANNEL( "TEST", "-0.2 * v1=" << vecToString( -0.2f * v1 ) );  
            LOG_CHANNEL( "TEST", "v1*=-0.2=" << vecToString(v3) ); 
        }
        LOG_CHANNEL( "TEST", "" );
        ...
        {
            LOG_CHANNEL( "TEST", "v1 cross v2=" << vecToString( vecCrossProduct( v1, v2 ) ) );
        }
        LOG_CHANNEL( "TEST", "" );
        ...
        LOG_CHANNEL( "TEST", "<<vec2 cross product test>>" );
        {
            ...
            LOG_CHANNEL( "TEST", "(vec2)v1 cross (vec2)v2=" << vecToString( vecCrossProduct( v3, v4 ) ) );
        }
        LOG_CHANNEL( "TEST", "" );
